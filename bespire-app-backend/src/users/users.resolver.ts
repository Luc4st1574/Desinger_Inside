...
import { FavoriteMember } from './dto/favorite-member.output';
import { User } from './entities/user.entity';
import { UpdateUserBasicInput } from './dto/update-user-basic.input';
import { TeamListAdmin, UserTeamMember } from './dto/user-team-member';
...
export class UsersResolver {
  ...
  createTeamMember(@Args('input') input: CreateTeamMemberByAdmin) {
    return this.usersService.createTeamMember(input);
  }

  @Mutation(() => User)
  updateUserBasicByAdmin(
    @Args('userId', { type: () => ID }) userId: string,
    @Args('input') input: UpdateUserBasicInput,
  ) {
    return this.usersService.updateUserBasicByAdmin(userId, input);
  }

  @Query(() => User)
  userById(@Args('userId', { type: () => ID }) userId: string) {
    return this.usersService.userById(userId);
  }

  @Query(() => UserTeamMember)
  GetUserTeamMember(@Args('userId', { type: () => ID }) userId: string) {
    return this.usersService.GetUserTeamMember(userId);
  }

  @Query(() => [User])
  teamMembersMini() {
    // retorna User con solo campos select; GraphQL igual expone el tipo completo,
    // pero al resolver vendrán esos campos. Puedes crear un tipo Mini si prefieres.
    return this.usersService.teamMembersMini() as any;
  }
  ...
  @Query(() => [TeamListAdmin], { name: 'getTeamListAdmin' })
  @UseGuards(GqlAuthGuard, PermissionsGuard)
  @Permissions(PERMISSIONS.MANAGE_USERS)
  async getTeamListAdmin(): Promise<TeamListAdmin[]> {
    return this.usersService.getTeamListAdmin();
  }
  ...
  @Query(() => [FavoriteMember], { name: 'GetClientFavoriteMembers' })
  @UseGuards(GqlAuthGuard, PermissionsGuard)
  @Permissions(PERMISSIONS.MANAGE_USERS)
  async GetClientFavoriteMembers(
    @Args('clientId', { type: () => String }) clientId: string,
  ): Promise<FavoriteMember[]> {
    //@ts-ignore
    return this.usersService.GetClientFavoriteMembers(clientId);
  }
  @Query(() => UserTeamMember, { name: 'getTeamMemberDetail' })
  @UseGuards(GqlAuthGuard, PermissionsGuard)
  @Permissions(PERMISSIONS.MANAGE_USERS)
  async getTeamMemberDetail(
    @Args('id', { type: () => String }) id: string,
  ): Promise<UserTeamMember | null> {
    return this.usersService.getTeamMemberDetail(id);
  }
  //getTeamMemberDetail
}

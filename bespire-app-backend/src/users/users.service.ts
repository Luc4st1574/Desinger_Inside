...
import { UpdateUserBasicInput } from './dto/update-user-basic.input';
...
export class UsersService {
  ...
  async refreshToken(userId: string) {
    ...
    console.log('Permissions for user:', user.firstName, userRole, permissions);
  ...
  async createTeamMember(input: CreateTeamMemberByAdmin) {
    const exists = await this.userModel.exists({ email: input.email });
    if (exists) throw new BadRequestException('Email already exists');

    const passwordHash = await this.createPasswordHash('passwordDefault**()&^');

    //tambien necesito el workspace id del admin
    const adminWorkspace = await this.getAdminWorkspaceSelected();

    //me viene ahora en el input fullName y quiero convertirlo a firstName y lastName
    const [firstName, lastName] = input.fullName.split(' ');
    const teamMember = new this.userModel({
      email: input.email,
      passwordHash,
      role: 'team_member',
      isInternalTeam: true,
      ...
      avatarUrl: null,
      workspaceSelected: adminWorkspace
        ? new Types.ObjectId(adminWorkspace)
        : null,
    });

    const newTeam = await teamMember.save();
    ...
    await this.workspaceService.SendActivateLink(input.email, 'Bespire');
    return teamMember;
  }

  async updateUserBasicByAdmin(userId: string, input: UpdateUserBasicInput) {
    const user = await this.userModel.findById(userId);
    if (!user) throw new NotFoundException('User not found');

    if (input.email && input.email !== user.email) {
      const dup = await this.userModel.exists({ email: input.email });
    ...
    if (typeof input.teamRole !== 'undefined')
      user.teamRole = input.teamRole as any;
    if (typeof input.avatarUrl !== 'undefined')
      user.avatarUrl = input.avatarUrl;

    await user.save();

    if (input.roleMember || input.titleRole || input.teamRole) {
      const ws = await this.workspaceModel.findById(user.workspaceSelected);
      if (!ws) throw new NotFoundException('Workspace not found');

      // localizar el miembro
      const member = ws.members?.find((m: any) =>
        typeof m.user?.equals === 'function'
          ? m.user.equals(user._id)
          : String(m.user) === String(user._id),
      );
      if (!member)
        throw new NotFoundException('User is not a member of the workspace');
      //@ts-ignore
      if (input.roleMember !== undefined) member.role = input.roleMember;
      if (input.titleRole !== undefined) member.title = input.titleRole;
      if (input.teamRole !== undefined) member.teamRole = input.teamRole;

      // marcar modificado y salvar validando sólo paths tocados
      ws.markModified('members');
      await ws.save({ validateModifiedOnly: true }); // ⬅️ clave
    }
  ...
  async GetUserTeamMember(userId: string) {
    const u = await this.userModel.findById(userId);
    if (!u) throw new NotFoundException('User not found');
    //ahora necesito los datos como miembro del workspace
    const workspace = await this.workspaceService.getWorkspaceById(
      u.workspaceSelected.toString(),
    );
    if (!workspace) throw new NotFoundException('Workspace not found');
    console.log('Workspace found:', workspace);
    //ahora los datos como miembro , recorrer
    //@ts-ignore
    const memberData = workspace.members.find((m) => m.user.equals(u._id));
    ...
    return {
      ...
      roleMember: memberData.role,
      title: memberData.title,
      avatarUrl: u.avatarUrl,
    };
  }

  async teamMembersMini() {
    return this.userModel
      .find({ role: 'team_member' })
  ...
  async getAllClientsWithWorkspaceInfo() {
    ...
    for (const client of clients) {
      ...
      clientsWithInfo.push({
        ...
        planId: workspace.plan ? workspace.plan.toString() : null,
        isWorkspaceOwner,
  ...
  async getTeamListAdmin() {
    const teamMembers = await this.userModel
      .find({ role: 'team_member', workspaceSelected: { $exists: true } })
      .exec();

    const teamMembersFullInfo = [];

    for (const team of teamMembers) {
      const workspace = await this.workspaceService.getWorkspaceById(
        team.workspaceSelected.toString(),
      );
      if (!workspace) continue;
      const memberInfo = workspace.members.find(
        (m: any) => m.user.toString() === team._id.toString(),
      );

      teamMembersFullInfo.push({
        id: team._id.toString(),
        name:
          `${team.firstName || ''} ${team.lastName || ''}`.trim() || team.email,
        email: team.email,
        avatarUrl: team.avatarUrl || null,
        roleTitle: memberInfo?.title || memberInfo.teamRole || null,
        workspaceId: workspace._id.toString(),
        rating: 4.8,
        kpi: 98,
        tasks: 15,
  ...
  async getClientListAdmin() {
    ...
    const pipeline = [
      {
        $match: {
          role: 'client',
          workspaceSelected: { $ne: null }, // Usamos $ne: null para asegurar que el campo existe y no es nulo.
        },
      },
      // ---  Unir con la colección de Workspaces ---
      // Usamos $lookup para hacer un "left join" a la colección de workspaces.
      {
        $lookup: {
          from: 'workspaces',
          localField: 'workspaceSelected',
          foreignField: '_id',
          as: 'workspaceDetails',
        },
      },

      // ---  Descomprimir el array del lookup ---
      // $lookup devuelve un array. Como sabemos que solo habrá un workspace,
      // usamos $unwind para convertir 'workspaceDetails' de un array a un objeto.
      // Si no se encontró un workspace, el documento del usuario se descarta aquí.
      {
        $unwind: '$workspaceDetails',
      },

      // ---  El Filtro Clave ---
      // Ahora que tenemos los datos del usuario y del workspace en el mismo documento,
      // podemos compararlos. Usamos $expr para comparar dos campos del mismo documento.
      {
        $match: {
          $expr: {
            $eq: ['$_id', '$workspaceDetails.owner'], // Compara el _id del usuario con el owner del workspace.
          },
        },
      },

      // ---  Limpieza ---
      // Removemos el campo temporal que creamos para que el resultado sea un objeto User limpio.
      {
        $project: {
          workspaceDetails: 0, // El 0 significa excluir este campo.
        },
      },
    ];

    // Ejecutamos el pipeline de agregación en el modelo de User.
    const ownerClients = await this.userModel.aggregate(pipeline).exec();
    const clientsWithInfo = [];
    ...
    for (const client of ownerClients) {
      ...
      const plan = await this.plansService.getPlanById(
        workspace.plan ? workspace.plan.toString() : null,
      );
      ...
      //@ts-ignore
      const lastSession = client.updatedAt ? client.updatedAt : null;
      console.log('Last session for user', client._id.toString(), lastSession);

      //ahora el isNew para saber si es un cliente nuevo o recurrente, podriamos sacarlo del createdAt del user
      //@ts-ignore
      const isNew = client
        ? //@ts-ignore
          client.createdAt > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
        : false;
      ...
      clientsWithInfo.push({
        ...
        timeRequest: timeRequest ? `${timeRequest} hours` : '0 hours',
        revisions: `${revisions.avg.toFixed(2)} revisions`,
  ...
  async resetPassword(token: string, newPassword: string): Promise<boolean> {
    ...
    user.registrationStatus = 'completed'; // Marcar como completado si estaba en progreso
  ...
  async getTeamMemberDetail(id: string) {
    console.log('getTeamMemberDetail', id);
    // Buscar usuario
    const teamMember = await this.userModel.findById(id).exec();
    if (!teamMember) return null;

    if (!teamMember.workspaceSelected) return null;

    const workspace = await this.workspaceService.getWorkspaceById(
      teamMember.workspaceSelected.toString(),
    );
    if (!workspace) return null;

    const memberInfo = workspace.members.find(
      (m: any) => m.user.toString() === teamMember._id.toString(),
    );

    const teamInfo = await this.teamService.teamMemberProfileByUser(id);
    let manager = null;

    console.log('teamInfo', teamInfo);
    if (teamInfo) {
      const managerinfo = await this.userModel.findById(
        teamInfo.manager.toString(),
      );
      if (managerinfo) {
        manager = {
          id: managerinfo._id.toString(),
          fullName:
            `${managerinfo.firstName || ''} ${managerinfo.lastName || ''}`.trim() ||
            managerinfo.email,
          email: managerinfo.email,
          avatarUrl: managerinfo.avatarUrl || null,
        };
      }
    }
    ...
    return {
      ...
      phone: teamInfo?.phone || null,
      firstName: teamMember.firstName || null,
      lastName: teamMember.lastName || null,
      avatarUrl: teamMember.avatarUrl || null,
      teamMemberRole: memberInfo?.teamRole || null,
      title: memberInfo?.title || null,
      description: teamInfo?.description || null,
      manager: manager,
      contractStart: teamInfo?.contractStart || null,
      contractEnd: teamInfo?.contractEnd || null,
      employmentType: teamInfo?.employmentType || null,
      timezone: teamMember.preferences?.timezone || null,
      country: teamInfo?.country || null,
      state: teamInfo?.state || null,
      city: teamInfo?.city || null,
      workHours: teamInfo?.workHours || null,
      birthday: teamInfo?.birthday || null,
      tags: teamInfo?.tags || [],
    };
  }
  ...
  //favorite members bespire
  async GetClientFavoriteMembers(clientId: string): Promise<string[]> {
    const client = await this.userModel.findById(clientId).exec();
    if (!client) return [];
    //aqui necesito 3 posibilidades para retornar
    //1) los users favorios a los que el cliente ha dado review , necesito un top3
    //2) si no hay favoritos del cliente entonces listar el top 3 de los mimebros de bespire general
    //3) si no hay favoritos de nada entonces intentar listar 3 miembros aleatorios de bespire
    //4) si no hay ninguna de las anteriores entonces retornar array vacio.
    const reviews = [];
    //1) opcion
    const favoriteReviews =
      await this.reviewsService.findTopUserReviewsByClient(clientId);
    console.log('favoriteReviews', favoriteReviews);
    if (favoriteReviews.length > 0) {
      reviews.push(...favoriteReviews);
    }
    const usersFavorites = [];
    //recorrer users
    for (const review of reviews) {
      //necesito el role del workspace y los servicios en los que ese user ha participado.
      const member = await this.workspaceService.getMemberDataWorkspace(
        review.workspaceId.toString(),
        review.id.toString(),
      );

      console.log('member for review', member);

      usersFavorites.push({
        ...review,
        //@ts-ignore
        role: member?.title || member?.role || 'unknown',
        services: ['UX/UX', 'Design System'],
      });
    }
    console.log('usersFavorites after favoriteReviews', usersFavorites);
    return usersFavorites;
  }
  async getAdminWorkspaceSelected(): Promise<string | null> {
    const adminUser = await this.userModel.findOne({ role: 'admin' }).exec();
    return adminUser ? adminUser.workspaceSelected.toString() : null;
  }

...
export class AssigneeService {
  constructor(
    ...
    private readonly requestVersionService: RequestVersionService, // Agrega esta inyección
  ) {}
  ...
  async addAssignee(
    ...
    const assigneeIds = [userObjectId];
    const snapshot: Partial<SnapshotData> = {
      assignees: assigneeIds,
    };

    await this.requestVersionService.create({
      requestId: linkedToIdObjectId,
      updatedBy: new Types.ObjectId(assignedBy),
      changedFields: ['Assignee Added'],
  ...
  async removeAssignee(
    ...
    if (result.deletedCount > 0) {
      ...
      await this.requestVersionService.create({
        requestId: linkedToIdObjectId,
        updatedBy: new Types.ObjectId(removedBy),
        changedFields: ['Assignee Removed'],

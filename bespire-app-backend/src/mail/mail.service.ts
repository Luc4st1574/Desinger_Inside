...
export class MailService {
  ...
  async sendTeamMemberUpdateData(
    email: string,
    clientName: string,
    companyName: string,
    dashboardLink: string,
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff;">
          ...
          <div style="padding: 40px 30px;">
            ...
            <p style="color: #4a5568; font-size: 16px; line-height: 1.6; margin-bottom: 20px;">
              We wanted to let you know that your team member profile information for <strong>${companyName}</strong> has been updated on Bespire.
            </p>
    ...
    try {
      const info = await this.transporter.sendMail(mailOptions);
      this.logger.log(`Team member update email sent: ${info.messageId}`);
    } catch (error) {
      this.logger.error('Error sending team member update email', error);
      throw new Error('Failed to send team member update email');
    }
  }
}

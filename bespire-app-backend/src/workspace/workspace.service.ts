...
export class WorkspaceService {
  ...
  async getMemberDataWorkspace(
    ...
    // Validar que el usuario sea owner o miembro del workspace
    const isMember = workspace.members.some((m) => m.user.equals(memberId));
    if (!isMember) {
      throw new ForbiddenException('You are not a member of this workspace');
    }
    //retornar solo el user del memberId
  ...
  async addMemberToWorkspace(
    ...
    // Usar update para solo agregar el miembro sin afectar otros campos requeridos
      ...
      { _id: workspace._id },
      {
        $push: {
          members: {
            ...
            role: roleMember as 'admin' | 'user' | 'viewer',
            teamRole: teamRole,
            title: titleRole,
            joinedAt: new Date(),
          },
  ...
  async findAdminsByWorkspace(workspaceId: string): Promise<User[]> {
    const workspace = await this.workspaceModel.findById(workspaceId);
    if (!workspace) throw new NotFoundException('Workspace not found');
    ...
    return this.usersService.findByIds(
      adminIds as unknown as import('mongoose').Schema.Types.ObjectId[],
    );
  }
}

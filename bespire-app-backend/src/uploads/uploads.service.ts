...
export class UploadsService {
  ...
  private readonly baseURL =
    process.env.FILES_API_BASE_URL?.replace(/\/+$/, '') || 'http://127.0.0.1:8090';
  private readonly apiKey = process.env.FILES_API_KEY || '';
  private readonly defaultProject = process.env.FILES_DEFAULT_PROJECT || 'default';

  async uploadImage(file: Express.Multer.File,user_id:string, fileData: any, project?: string, ): Promise<UploadResponse> {
    ...
    if (!file.buffer?.length) {
      ...
      throw new BadRequestException('Empty file buffer. Ensure Multer uses memory storage.');
    }
    ...
    console.log('FormData prepared, making request to:', `${this.baseURL}/v1/upload`);

    try {
      const res = await axios.post<UploadResponse>(`${this.baseURL}/v1/upload`, form, {
      ...
      console.log('File created in database with workspaceId:', user.workspaceSelected.toString());
     ...
     //añadir el id del file en la respuesta
      ...
      const msg = err?.response?.data?.detail || err?.message || 'Unknown upload error';
  ...
  async deleteByKey(key: string): Promise<{ ok: boolean; deleted: string }> {
    try {
      const res = await axios.delete<{ ok: boolean; deleted: string }>(
        ...
        { params: { key }, headers: { 'X-API-Key': this.apiKey }, timeout: 20_000 },
      );
      return res.data;
    } catch (err: any) {
      const msg = err?.response?.data?.detail || err?.message || 'Unknown delete error';

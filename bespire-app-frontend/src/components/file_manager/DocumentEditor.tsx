...
export default function DocumentEditor({ allFiles, currentFolderId, workspaceId }: DocumentEditorProps) {
    console.log('DocumentEditor rendered with workspaceId:', workspaceId);
    const { editorTags = [], setEditorTags, setEditorContent, editorContent, user } = useAppContext();
    console.log('DocumentEditor context:', { editorTags, editorContent, user });
    // Inicializar con el contenido del contexto una sola vez
    const [initialContent] = useState(() => editorContent || "");
    const { state: content, setState: setContent, undo, redo, canUndo, canRedo } = useHistory<string>(initialContent);
    ...
    useEffect(() => {
        ...
        // Manejar contenido inicial: puede ser HTML simple o JSON con delta/html
        if (content) {
            try {
                const parsed = JSON.parse(content);
                if (parsed.delta) {
                    //console.log('Loading content from Delta:', parsed.delta);
                    quill.setContents(parsed.delta);
                } else if (parsed.html) {
                    //console.log('Loading content from HTML in JSON:', parsed.html);
                    quill.clipboard.dangerouslyPasteHTML(parsed.html);
                } else {
                    //console.log('Loading content as HTML fallback from JSON');
                    quill.clipboard.dangerouslyPasteHTML(content);
                }
            } catch {
                // Si no es JSON válido, tratarlo como HTML simple
                //console.log('Loading content as plain HTML:', content);
                quill.clipboard.dangerouslyPasteHTML(content);
            }
        }
        ...
        quill.on('text-change', (delta, oldDelta, source) => {
            if (source === 'user') {
                ...
                const semanticHTML = quill.getSemanticHTML ? quill.getSemanticHTML() : htmlContent;
                const deltaContent = quill.getContents();
                //console.log('Quill root.innerHTML:', htmlContent);
                //console.log('Quill getSemanticHTML:', semanticHTML);
                //console.log('Quill Delta:', JSON.stringify(deltaContent));
                
                // Guardar tanto el HTML como el Delta para tener ambas opciones
    ...
    // Sincronizar el contenido con el contexto global para que pueda ser guardado
    // Usar un debounce para evitar actualizaciones excesivas
    const lastSyncedContentRef = useRef<string>('');
    useEffect(() => {
        if (content !== lastSyncedContentRef.current) {
            lastSyncedContentRef.current = content;
            //console.log('Syncing content to context:', content);
            ...
            <div className="flex justify-between items-center flex-shrink-0 px-2 -mt-6">
                ...
                <div className="hidden md:flex items-center gap-3 text-sm text-gray-500">
                    {user?.avatarUrl ? (
                        ...
                        <img
                            className="h-8 w-8 rounded-full"
                            src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'User')}&background=e5e7eb&color=374151&size=40`}
                            alt={user?.name || 'User'}
                            width={32}
                            height={32}
                        />

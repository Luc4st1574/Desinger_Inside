...
export default function RequestOverviewSidebar({ request, onBackToMain }: RequestOverviewSidebarProps) {
  ...
  function handleInternalDueDateChange(newDate: Date | null) {
    if (newDate) {
  ...
  function handleDueDateChange(newDate: Date | null) {
    if (newDate) {
      console.log("Updating due date to:", newDate);
      updateFields({ dueDate: newDate });
    }
  }
  ...
  return (
    <aside className="w-70 min-w-[260px] p-6 border-r border-[#E2E6E4] flex flex-col gap-2 overflow-y-auto">
      ...
      <div className="flex flex-col gap-1 mt-2">
        ...
        <div className="font-medium text-[#181B1A]">{request?.service?.category?.name}</div>
        <div className="flex items-center gap-2">
          <span className="ml-2 text-2xl">↳</span>
          <span className="text-[#181B1A]">{request?.service?.title}</span>
        </div>
      </div>
      {/* Credits */}

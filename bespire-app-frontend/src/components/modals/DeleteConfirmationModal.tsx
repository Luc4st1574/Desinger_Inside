...
export default function DeleteConfirmationModal({ 
    ...
    const title = isPermanent 
        ? `Permanently delete ${itemCount} ${itemCount === 1 ? (isFolder ? 'Folder' : 'File') : (isFolder ? 'Folders' : 'Files')}?`
        : `Delete ${itemCount} ${itemCount === 1 ? (isFolder ? 'Folder' : 'File') : (isFolder ? 'Folders' : 'Files')}?`;

    let description = '';
    if (isPermanent && isFolder) {
        description = "This will permanently delete the folder and all its contents. This action cannot be undone.";
    } else if (isPermanent) {
        description = "This action cannot be undone. The selected files will be deleted forever.";
    } else {
        description = `${itemCount} ${itemCount === 1 ? (isFolder ? 'folder' : 'file') : (isFolder ? 'folders' : 'files')} selected will be sent to trash. You can restore it anytime from the Trash section.`;
    }

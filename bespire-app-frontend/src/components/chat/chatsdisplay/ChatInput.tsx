...
const AttachmentPreviewArea = ({ attachments, onRemove }: { attachments: Attachment[]; onRemove: (id: string) => void; }) => {
    return (
        <div className="bg-white flex flex-wrap items-start gap-2 p-3">
            {attachments.map(att => {
...
// ✅ Helper to get a plain text preview of a message being replied to
const getReplyPreview = (text: ConversationMessage['text']) => {
    ...
    if (text && typeof text === 'object' && 'root' in text && (text as any).root.children.length > 0) {
        const firstNode = (text as any).root.children[0];
        if (firstNode && firstNode.children.length > 0) {
            return firstNode.children[0]?.text || '[Attachment]';
        }
    }
    return '[Attachment]';
};
...
interface ChatInputProps {
    onSendMessage: (text: object | null, attachments: Attachment[]) => void;
    onAttachmentPreviewToggle: (isPreviewVisible: boolean) => void;
    replyingTo: ConversationMessage | null; // ✅ Add prop
    onCancelReply: () => void; // ✅ Add prop
}

export default function ChatInput({ onSendMessage, onAttachmentPreviewToggle, replyingTo, onCancelReply }: ChatInputProps) {
    const fileInputRef = useRef<HTMLInputElement>(null);
    const [attachments, setAttachments] = useState<Attachment[]>([]);

    useEffect(() => {
        // This effect still correctly tells the parent component if the input is expanded in any way
        onAttachmentPreviewToggle(attachments.length > 0 || !!replyingTo);
    }, [attachments, replyingTo, onAttachmentPreviewToggle]);
    ...
    function ActionsPlugin() {
        ...
        const onSend = React.useCallback(() => {
            ...
            editor.focus();
        }, [canSend, editor]); // Fixed dependencies
    ...
    return (
        <div className={`relative z-10 px-6 bg-white transition-all duration-300 ease-in-out ${getMarginClass()}`}>
            <div className="border border-gray-300 rounded-lg overflow-hidden bg-white shadow-sm">
                
                {replyingTo && (
                    <div className="bg-[#f1f3ee] px-4 py-3 flex items-start justify-between gap-4">
                        <div className="flex-grow min-w-0">
                            <div className="flex items-center gap-2">
                                <ShareIcon className="w-4 h-4 text-[#353b38]" />
                                <p className="text-sm font-semibold text-black">Replying to</p>
                            </div>
                        ...
                        <button 
                            ...
                            className="p-1 rounded-full hover:bg-gray-400/20 flex-shrink-0"
                            title="Cancel reply"
                        >

...
import CommonPhrasesBadge from "./CommonPhrasesBadge";
...
const MemberCard: React.FC<MemberCardProps> = ({ member, className = "" }) => {
  ...
  return (
    <div
      className={`border border-gray-200 rounded-lg p-2 md:p-4 ${className}`}
    >
      <div className="flex flex-col items-center text-center gap-2 ">
        ...
        <p className="text-xs text-green-gray-500 capitalize">{member.role}</p>
        ...
        <div className="flex flex-wrap gap-1 justify-center">
          {member.services.map((service: string, idx: number) => (
            <CommonPhrasesBadge phrase={service} variant="colored" key={idx} />
          ))}
        </div>
      </div>
    </div>

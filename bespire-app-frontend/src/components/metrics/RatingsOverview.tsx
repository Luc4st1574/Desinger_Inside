...
import { ChevronDown, ChevronRight, X } from 'lucide-react'; // Import X icon
...
const RatingsOverview = ({ tasks }: RatingsOverviewProps) => {
  ...
  return (
    <div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-md"> {/* Added shadow-md here */}
        ...
        <div className="grid grid-cols-[auto_1fr] grid-rows-[1fr_auto] h-56 mt-8">
          <div className="col-start-1 row-start-1 flex flex-col justify-between pr-4 text-right">
            {[5, 4, 3, 2, 1, 0].map(label => (<div key={label} className="text-xs text-gray-400">{label}</div>))}
          </div>
          <div className="col-start-2 row-start-1 relative">
            {[0, 1, 2, 3, 4, 5].map(level => (<div key={level} className={`absolute left-0 right-0 h-px ${level === 0 ? 'bg-gray-300' : 'bg-gray-100'}`} style={{ bottom: `calc(${level / 5 * 100}%)` }}></div>))}
          ...
          <div className="col-start-2 row-start-2 grid gap-2 mt-1" style={{ gridTemplateColumns: `repeat(${Math.max(chartData.length, 1)}, minmax(0, 1fr))` }}>
            {chartData.map((data) => (<div key={data.name} className="text-center text-xs text-[#6e7b75] pt-2 truncate">{data.name}</div>))}
          </div>
        </div>
      ...
      <Transition appear show={isModalOpen} as={Fragment}>
        <Dialog as="div" className="relative z-50" onClose={() => setIsModalOpen(false)}>
          ...
          <div className="fixed inset-0 overflow-y-auto">
            {/* NEW: Close button positioned at the top right */}
            <button
              type="button"
              onClick={() => setIsModalOpen(false)}
              className="absolute top-4 right-4 z-10 text-white opacity-75 hover:opacity-100 focus:outline-none rounded-full p-1"
              aria-label="Close"
            >
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                <Dialog.Panel className="w-full max-w-6xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-2xl transition-all"> {/* Added shadow-2xl here */}
                  ...
                  <div className="grid grid-cols-[auto_1fr] grid-rows-[1fr_auto] h-96 mt-8 shadow-md"> {/* Added shadow-md here for the modal chart */}
                    <div className="col-start-1 row-start-1 flex flex-col justify-between pr-4 text-right">
                      {[5, 4, 3, 2, 1, 0].map(label => (<div key={label} className="text-xs text-gray-400">{label}</div>))}
                    </div>
                    <div className="col-start-2 row-start-1 relative">

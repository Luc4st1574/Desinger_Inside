...
const ClientRequestsTab: React.FC<ClientRequestsTabProps> = ({ client }) => {
  console.log("Rendering ClientRequestsTab for client:", client);
  const [activeTab, setActiveTab] = useState("all");

  const { requests, loading, error, refetch } = useRequestsByAdmin({ userClientId: client?.id ?? undefined });
  ...
  return (
    <div className="bg-white rounded-lg border border-[#bcbcbc47] max-w-full overflow-x-auto">
     ...
     <RequestsTabs
      ...
      {/* Table */}
      <div className="p-0">
        <ClientRequestsTable requests={filteredRequests} onSetRequest={(request) => {
